...
template<class T> int person<T>::send_registration(int day, int time,int treament_type) {
...
bool Report::store_to_file(person<int> p) {
    /*  
        store a person's information to file. The target filw is ID.txt, where ID
        is the person's ID

        p: person<int>, the person you want to store
    ...
    */
    ...
    // set the file mode to overwritten
...
bool Report::store_to_file_auto(person<int> p) {
    /*  
        store a person's information to file. The target filw is ID.txt, where ID
        is the person's ID
    ...
    */
    ...
    // set the file mode to overwritten
    ofstream fout("./database_auto/" + filename, ios::trunc);
...
bool Report::_display_report(int timenow, int range, string sort_by, bool is_summary, bool where, bool save) {
    vector<person<int>> report_list;
    vector<string> filenames;
    if (where) GetFileNames("./database", filenames);
    else GetFileNames("./database_auto", filenames);
    ...
    // gather all people's information into report_list
    ...
    print_split_line();
    for (int i = 0; i < int(report_list.size()); i++) {
        if (day_between(report_list[i].treat_date, timenow) <= range && report_list[i].treated && (report_list[i].appoint.day < timenow)) {
            cnt ++;
    ...
    print_split_line();
    for (int i = 0;i < int(report_list.size()); i++) {
        if (day_between(report_list[i].regist_date, timenow) <= range && !report_list[i].treated && !report_list[i].withdrawal && (report_list[i].appoint.day < timenow)) {
            cnt ++;
            cout << "********* Person" << cnt << " *********" << endl;            
...
bool Report::display_weekly_report(int timenow, string sort_by, bool where) {
    this -> _display_report(timenow, 7, sort_by, false, where, true);
    return this -> _display_report(timenow, 7, sort_by, false, where, false);
}

bool Report::display_monthly_report(int timenow, string sort_by, bool where) {
    this -> _display_report(timenow, 30, sort_by, true, where, true);
    return this -> _display_report(timenow, 30, sort_by, true, where, false);
}
...
template <class T> void FibonacciHeap<T>::Display() {
    ...
    do {
        i++;
        cout << "#########################" << endl;
        cout << i << ". " << (p -> fib_person) -> return_name() << " (degree: " << p -> degree << ") is root.";
        cout << " (Priority: " << (p -> fib_person) -> return_priority() << endl;
        print(p -> child, p, 1);
        p = p -> right;
    } while (p != H);
    cout << endl;
